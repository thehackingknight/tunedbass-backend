...
router.get("/check", passport.authenticate("jwt"), async (req, res) => {
  ...
  if (isAuthenticated) {
    ...
    let plays = 0,
      likes = 0,
      shares = 0;
    for (let trackId of req.user.tracks) {
      // Clean up each track
...
router.post("/signup", async (req, res) => {
  ...
  if (username && email && password) {
    let userWithSameCredentials = await User.findOne({
      username,
      email,
    }).exec();
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!user)
      return res
        .status(401)
        .json({ msg: "No user found with the specified email" });
    if (!user.is_verified)
      return res
        .status(400)
        .json({ msg: "Please verify your account or signup to continue" });
    if (await bcrypt.compare(password, user.password)) {
      let token = jwt.sign(
        { id: user.id, exp: Date.now() + 24 * 60 * 60 * 1000 },
